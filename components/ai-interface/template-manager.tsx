...
export function TemplateManager() {
  ...
  const categories = ["all", ...new Set(state.templates.map((t) => t.category))]
    ...
    <div className="space-y-4 sm:space-y-6">
          ...
          <DialogContent className="w-[95vw] max-w-2xl h-[90vh] max-h-[90vh] sm:h-[80vh] sm:max-h-[80vh]">
            ...
            <Tabs defaultValue="basic" className="w-full flex-1 min-h-0">
              ...
              <div className="flex-1 min-h-0 overflow-hidden">
                <TabsContent value="basic" className="space-y-3 sm:space-y-4 h-full overflow-y-auto">
                ...
                <TabsContent value="parameters" className="space-y-3 sm:space-y-4 h-full overflow-y-auto">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    <div className="space-y-2">
                      <Label className="text-sm">Temperature: {newTemplate.parameters?.temperature}</Label>
                      <Slider
                    ...
                    <div className="space-y-2">
                      <Label className="text-sm">Max Tokens: {newTemplate.parameters?.maxTokens}</Label>
                      <Slider
                    ...
                    <div className="space-y-2">
                      <Label className="text-sm">Top P: {newTemplate.parameters?.topP}</Label>
                      <Slider
                    ...
                    <div className="space-y-2">
                      <Label className="text-sm">Frequency Penalty: {newTemplate.parameters?.frequencyPenalty}</Label>
                      <Slider
                        value={[newTemplate.parameters?.frequencyPenalty || 0.0]}
                        onValueChange={([value]) =>
                          setNewTemplate((prev) => ({
                            ...prev,
                            parameters: { ...prev.parameters!, frequencyPenalty: value },
                          }))
                        }
                        max={2}
                        min={-2}
          ...
          <Card key={template.id} className="hover:shadow-md transition-shadow">
            <CardHeader className="pb-2 sm:pb-3">
              <div className="flex items-start justify-between">
                <div className="space-y-1 min-w-0 flex-1">
                  ...
                  <CardDescription className="text-xs sm:text-sm line-clamp-2">{template.description}</CardDescription>
                </div>
            ...
            <CardContent className="space-y-2 sm:space-y-3">
              ...
              <div className="text-xs sm:text-sm text-muted-foreground">
                Temp: {template.parameters.temperature} • Tokens: {template.parameters.maxTokens}
              </div>

              <Button className="w-full text-sm" onClick={() => loadTemplate(template)}>
                Use Template
              </Button>
            </CardContent>
          </Card>
      ...
      {filteredTemplates.length === 0 && (
        <div className="text-center py-8 sm:py-12">
          <p className="text-muted-foreground text-sm">No templates found matching your criteria.</p>
        </div>
      )}
    </div>
  )
