...
export function AIProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        const [modelsRes, templatesRes] = await Promise.all([fetch("/api/models"), fetch("/api/templates")])
        ...
        if (savedModelId) {
          modelToSelect = modelsData.models.find((m: AIModel) => m.id === savedModelId && m.isAvailable)
        }
  ...
  const loadTemplate = useCallback((template: PromptTemplate) => {
    ...
    dispatch({ type: "SET_SYSTEM_PROMPT", payload: template.systemPrompt || "" })
  ...
  }, [state.messages, state.selectedModel, state.parameters, state.systemPrompt])
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          try {
            ...
            dispatch({ type: "SET_ERROR", payload: "Failed to load conversation file" })
          }
        }
